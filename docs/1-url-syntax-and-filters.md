...
The package seamlessly supports two distinct formats for URL parameters. You can
use whichever format best fits your frontend application.

## 1. JSON Syntax

You can pass raw JSON strings directly into the URL parameter. This is ideal for
modern JavaScript frontends that easily serialize objects.
...
The standard PHP/Laravel nested array syntax. This is ideal for traditional HTML
forms or programmatic URL generation.
...
_(Note: PostgreSQL-specific operators securely abort with an
`InvalidArgumentException` if executed on non-PostgreSQL engines to prevent raw
SQL syntax errors)._
